...
{
    HashTable *table = (HashTable *)conf->mem_calloc(1, sizeof(HashTable));

    if (!table)
        return CC_ERR_ALLOC;

    table->capacity = round_pow_two(conf->initial_capacity);
	table->buckets = (TableEntry **)conf->mem_calloc(table->capacity, sizeof(TableEntry *));
...
enum cc_stat hashtable_add(HashTable *table, void *key, void *val)
{
    ...
    TableEntry *new_entry = (TableEntry *)table->mem_alloc(sizeof(TableEntry));
...
static enum cc_stat resize(HashTable *t, size_t new_capacity)
{
    ...
    TableEntry **new_buckets = (TableEntry **)t->mem_calloc(new_capacity, sizeof(TableEntry *));
